...
public class Notify4AliPayController extends Notify4BasePay {
	...
	public String doAliPayRes(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (trade_status==null && params.get("method").equalsIgnoreCase("alipay.trade.page.pay.return")) {
			trade_status = PayConstant.AlipayConstant.TRADE_STATUS_SUCCESS;
		}
		// 支付状态成功或者完成
	...
	public boolean verifyAliPayParams(Map<String, Object> payContext) {
		...
		try {
			verify_result = AlipaySignature.rsaCheckV1(params, alipayConfig.init(payChannel.getParam()).getAlipay_public_key(), AlipayConfig.CHARSET, AlipayConfig.SIGNTYPE);
